...
const getSentMails = async (req, res) => {
  try {
    const userId = req.user._id; // Assumes that user is authenticated and user data is available in req.user

    // Fetch all sent mails for the authenticated user
    const sentEmails = await Email.find({ from: userId });
